...
kmer_dat
p <- ggplot(kmer_dat, aes(Genome, Reads)) + geom_point()
p
```

Looks like the mean frequency of a k-mer in the reads is not a bad estimator of its frequency in the source genome. There is some error though. Let's see if we can reduce the error by increasing the number of reads (in other words, increase the read "coverage").
...
p <- ggplot(kmer_dat10000, aes(Genome, Reads)) + geom_point()
p
```
